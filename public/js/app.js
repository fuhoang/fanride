$(function(){
  ...
  if($('#selectSports').length){
	...
	apiurl = 'http://fanride.local/sports/';
	$.ajax({
    	url: apiurl,
        type: 'GET',
        crossDomain: true,
        success: function (returnData) {
        	...
        	var html = [];
            html.push('<option value="">Select a Sports</option>');
        	...
        	$.each(returnData, function (key, value) {
        		...
        		html.push('<option value=' + value.id + '>' + value.sport + '</option>');
        	});
        	jQuery('#selectSports').html(html.join(''));
        	$('#selectSports').selectpicker('refresh');
		...
		$.ajax({
	        ...
	        success: function (returnData) {
	        	...
	        	$.each(returnData, function (key, value) {
	        		...
	        		//jQuery('.fixtures-list').append('<a href="' + urlResult	 + '"  class="list-group-item">'+ value.fixture +'</a>');
	        		html.push('<option value=' + value.id + '>' + value.fixture + '</option>');
	        	});
	        	jQuery('#selectFixtures').html(html.join(''));
	        	jQuery('#selectFixtures').selectpicker('refresh');
	        },
	        error: function (exception) {
	            console.log(exception);
	        }
